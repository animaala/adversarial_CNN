...
for i in range(nSteps):
    ...
    s, k = sess.run([merged_summary, train_step], feed_dict={X: batch_xs, Y_: batch_ys, lr: 0.004, pkeep: 0.5})
    ...
    if (i + 1) % 100 == 0:  # then perform validation
        ...
        train_accuracy = accuracy.eval(feed_dict={X: vbatch_xs, Y_: vbatch_ys, lr: 0.004, pkeep: 1.0})
